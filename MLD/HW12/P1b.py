...
X = forwardProp(x[0],W_all,L,identity_v)
h = X[-1]
# Each weight element is iterated over and perturbed using Identity
# Gradient is then calculated
for i in range(len(W_all)):
	for j in range(len(W_all[i])):
		for k in range(len(W_all[i][j])):
			W_all[i][j,k] +=  0.0001
			h_1 = forwardProp(x[0],W_all,L,identity_v)[-1]
			W_all[i][j,k] -=  2*0.0001
			h_2 = forwardProp(x[0],W_all,L,identity_v)[-1]
			G_all[i][j,k] = (h_1-h_2)/0.0002
			W_all[i][j,k] +=  0.0001
print(G_all)

X = forwardProp(x[0],W_all,L,tanh_v)
h = X[-1]
# Each weight element is iterated over and perturbed using Identity
# Gradient is then calculated
for i in range(len(W_all)):
	for j in range(len(W_all[i])):
		for k in range(len(W_all[i][j])):
			W_all[i][j,k] +=  0.0001
			h_1 = forwardProp(x[0],W_all,L,tanh_v)[-1]
			W_all[i][j,k] -=  2*0.0001
			h_2 = forwardProp(x[0],W_all,L,tanh_v)[-1]
			G_all[i][j,k] = (h_1-h_2)/0.0002
			W_all[i][j,k] +=  0.0001
print(G_all)
